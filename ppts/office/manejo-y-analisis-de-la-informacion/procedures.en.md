...
This is the click-by-click record of how each of the 107 Microsoft Office Specialist objectives is performed in Excel (59 for MO-200 Associate, 48 for MO-201 Expert. It exists because the certification does not grade the result, it grades the process. The professor's own example is the one to keep in mind: changing font colour and fill is two clicks on the Home tab for any normal user, and the exam is watching for something else entirely) that the candidate opens the extended menu and applies both at the same time. Same sheet at the end, different score.

The objective domain already carries that distinction. MO-200 lists 2.2.5 "Apply number formats" and 2.2.6 "Apply cell formats from the Format Cells dialog box" as two separate objectives precisely because they are two routes to the same picture, and only one of them is what 2.2.6 is asking for.

Use the document two ways. When you are building a deck, the exam route is the slide: the numbered steps are what goes on screen, and the short route is the slide after it, the one that says "this is what you will do at work, and this is why it earns nothing on exam day". When you are grading an exercise, go straight to "How to check it", every objective carries a test you can run on the finished file that separates the graded route from the fast one, and most of them take under ten seconds.

The interface is named in English throughout, because the professor's Excel runs an English UI (Microsoft 365, build 16.0.20228, UI language 1033, install language Spanish Mexico 2058) and every caption below could therefore be checked against the real product. Where a caption could not be read back it says **TO CONFIRM**, and it is listed again at the end. The Spanish version is a separate file, `procedures.es.md`, built from this one.
...
**Short route, not graded**

Drag the .txt onto Excel, or use **File**, **Open**, **Browse** with the file-type filter set to text files. Excel guesses the delimiter and the data types, and it usually guesses right on a clean tab-delimited file, which is exactly why it teaches nothing: a leading-zero product code arrives as a number and loses its zeros, and the task that asked for a text column is failed even though the sheet looks populated. The second short route is Ctrl+C from Notepad, Ctrl+V into the sheet, then **Text to Columns**. That reaches the same wizard through the wrong door, and the exam is looking at the import.

**Shortcut**

None for the wizard itself. `Alt`, `A`, `P`, `N` walks the Data tab to Get Data by key tips once the legacy entry exists. `Ctrl+O` reaches the Open browser for the short route.

**How to check it**

Click any cell in a column you set to **Text** and read the Formula Bar. A value like `00734` still carrying its leading zeros, left-aligned in the cell, means the wizard applied the format; numbers imported as text sit left, numbers imported as numbers sit right. Then go to the **Data** tab, **Queries & Connections**, and confirm nothing is listed. The legacy wizard pastes a static block and creates no query, whereas the From Text/CSV route of 1.1.2 does create one, and that difference is the fastest way to tell which route a student actually took.

**Where it lands**

Week 9, session 17, as an addition. COBERTURA.md lists 1.1.1 among the additions to that session and gives the reason plainly: Get and Transform already covers the .csv branch, so what is missing is the wizard branch, delimited against fixed width, the delimiter choice, the encoding. Practised in Exercise 14.
...
Press `Ctrl+F`, type the string, press `Enter` repeatedly. The dialog stays collapsed, the search stays on the active sheet, and **Look in:** stays on its last value, which may be **Formulas** from a previous session and produce hits the student cannot explain. Scrolling and reading is the other short route, and on the 26,608-row file of Exercise 23 it is not a route at all.

**Shortcut**

`Ctrl+F` opens **Find and Replace** on the **Find** tab. `Ctrl+H` opens the same dialog on the **Replace** tab. `Shift+F4` repeats the last find without reopening the dialog. `Ctrl+A` inside the **Find All** result list selects every match.

**How to check it**

Reopen the dialog. Excel remembers **Within:**, **Search:**, **Look in:** and the check boxes between openings within a session, so their state is the record of what was set. Then confirm the count: the **Find All** list footer reports how many cells were found, and if **Within:** was left on **Sheet** in a multi-sheet workbook that count will be short.

**Where it lands**

Week 3, session 6, as an addition. COBERTURA.md flags this one hard: none of the 34 sessions names it, and the only "find and replace" in the syllabus is the Word one in session 2. It has to be taught before formulas start.
...
Open the **Name Box** at the left end of the Formula Bar and pick the name from its drop-down list, or type an address into it and press `Enter`. It is faster and every professional uses it. It also cannot reach a single workbook element: there is no Name Box entry for "every cell that carries a data validation rule". Scrolling to the range and dragging over it is the other short route, and it fails the moment the task says "select the named range" rather than "look at it", because a hand-drawn selection that is one row off looks identical on screen.

**Shortcut**

`F5` or `Ctrl+G` for **Go To**. `Ctrl+G` then `Alt+S` opens **Go To Special** in two keystrokes. `F5` then `Enter` returns to the previous position, because Go To keeps a history of the last four jumps in the **Go to:** list.

**How to check it**

After the jump, read the **Name Box**. If it shows the name rather than a cell address, a genuine named range is selected and the whole range is highlighted. If it shows an address like `B2`, either the name does not cover what you think or only one cell got selected. For the Special route, check the Status Bar: it reports **Count** for the number of cells actually selected, which is how you prove that "select all blanks" caught all of them.

**Where it lands**

Week 3, session 6, as an addition. COBERTURA.md separates this from 2.3.1 explicitly: defining a name and navigating to it are two different objectives, and the syllabus only covers defining. Exercise 2 already practises the Name Manager, so the navigation half sits naturally beside it.
...
Type `www.something.com` into a cell and press `Enter`. AutoCorrect converts it to a link on its own. There is no ScreenTip, the display text is the raw URL, and nothing was demonstrated. To edit, right-click and click **Edit Hyperlink...**, which reopens the same dialog and is a perfectly good habit, just not the insertion the task asked for.

**Shortcut**

`Ctrl+K` opens **Insert Hyperlink** for a new link and **Edit Hyperlink** for an existing one. To select a linked cell without following the link, click and hold on it rather than clicking, or arrow into it from a neighbouring cell.

**How to check it**

Hover over the cell without clicking. The ScreenTip you typed must appear in the tooltip; if the raw address appears instead, step 8 was skipped. Then right-click the cell: **Edit Hyperlink...**, **Open Hyperlink** and **Remove Hyperlink** are present only while a real hyperlink object exists on the cell. After removal all three disappear from the menu even if the text is still blue and underlined, which is exactly the difference between **Clear Hyperlinks** and **Remove Hyperlinks**.

**Where it lands**

Week 5, session 9, already in the syllabus, the session reads "conditional formatting, icon sets, sparklines, hyperlinks". COBERTURA.md counts 1.2.3 as covered. Exercise 3 builds four kinds of hyperlink.
...
Use the galleries on the **Page Layout** tab one at a time: **Margins**, **Orientation**, **Size**, **Breaks**, **Background...**, and the **Width:**, **Height:** and **Scale:** boxes in the Scale to Fit group. Every one works, and each is a separate operation with its own undo step. The exam asks for a page setup, singular, and the dialog is where a page setup is one thing.

**Shortcut**

No direct key. `Ctrl+P` opens the Print backstage; the **Page Setup** link at the bottom of the Settings column opens the same dialog with some **Sheet** tab options greyed, because that path cannot set the print area. Prefer the ribbon launcher for that reason. `Alt`, `P`, `S`, `P` opens it by key tips, and `Ctrl+Tab` walks the four tabs.

**How to check it**

Switch to the **View** tab and click **Page Layout**. Margins, orientation and the repeated title rows are all visible on screen there. For scaling, the Status Bar in **Page Break Preview** shows the page count; setting **Fit to: 1 page(s) wide by** blank **tall** should collapse a wide sheet to one column of pages while leaving the vertical page count alone. Confirmed in the object model: that state is `Zoom = False`, `FitToPagesWide = 1`, `FitToPagesTall = False`, and if `Zoom` is still a number the Fit to option never took.

**Where it lands**

Week 17, session 33, already in the syllabus: "page setup, print area, repeating row and column titles, sheet options". Exercise 4 runs it on a 196-country, 35-column base that prints on twenty pages untouched.
...
Drag the border between two row headings or two column headings; double-click that same border to autofit. Or right-click the heading and click **Row Height...** or **Column Width...** from the context menu, which are the same dialogs reached by a shorter path. Dragging is the one that costs marks: a task that says "set the width of column C to 18" cannot be satisfied by a drag, because 18 is not a position the mouse can find.

**Shortcut**

`Alt`, `H`, `O`, `H` for Row Height. `Alt`, `H`, `O`, `W` for Column Width. `Alt`, `H`, `O`, `A` for AutoFit Row Height. `Alt`, `H`, `O`, `I` for AutoFit Column Width. There is no Ctrl combination for any of the four.

**How to check it**

Reopen **Format**, **Row Height...** or **Column Width...** and read the number in the box. It shows the exact stored value, and for a multi-row selection it shows blank when the rows differ, which proves at a glance whether the whole selection was hit. Second check: after **AutoFit Column Width**, no cell in the column shows `#####` and no text is clipped at the right edge.

**Where it lands**

Week 3, session 5, already in the syllabus: "adjust height and width". COBERTURA.md counts 1.3.2 as covered and warns that the "fit cells" of session 4 is ambiguous and should not be read as this objective.
...
Go to the **Insert** tab, **Text** group, and click **Header & Footer...**. Excel switches to Page Layout view, shows three click-in boxes at the top of the page, and adds a **Header & Footer** contextual tab carrying the same field buttons plus **Go to Header** and **Go to Footer**. It is easier to see what you are doing and it is a legitimate way to work. It is not the dialog, and the check boxes for odd and even pages sit on that contextual tab rather than in a group, which turns "apply all four at once" into a click sequence instead of one operation.

**Shortcut**

None. `Alt`, `P`, `S`, `P` opens **Page Setup** by key tips, then `Ctrl+Tab` walks to the **Header/Footer** tab.

**How to check it**

Switch to the **View** tab, **Workbook Views** group, and click **Page Layout**. The header and footer render on the page as they will print. If **Different first page** was ticked, scroll to page two and confirm the header there differs from page one. To check the codes rather than the rendering, reopen **Custom Header...**: the boxes show `&[Page]`, `&[Date]`, `&[File]` and so on, and a header typed as literal text instead of inserted as a field shows the plain words and will never update.

**Where it lands**

Week 17, session 33, as an addition. COBERTURA.md is direct about it: the only "headers and footers" in the syllabus are the Word ones in session 2, and in Excel they live in the same Page Setup dialog the session already opens for the print area. Exercise 4 does not touch them, so this needs new material.
...
Click the **Customize Quick Access Toolbar** arrow at the end of the toolbar and tick items off its short checklist: **New**, **Open**, **Save**, **Email**, **Quick Print**, **Print Preview and Print**, **Spelling**, **Undo**, **Redo**, **Sort Ascending**, **Sort Descending**, **Touch/Mouse Mode**, **More Commands...**, **Show Below the Ribbon**. Or right-click any ribbon button and click **Add to Quick Access Toolbar**, the trick every experienced user uses, and genuinely the best way to add a button you can already see. It reaches nothing under **Commands Not in the Ribbon**, it cannot scope to one workbook, and it cannot reorder.

**Shortcut**

`Alt+F`, `T` opens Excel Options. Once a button sits on the toolbar it answers to `Alt+1`, `Alt+2` and so on by position, which is worth showing because it explains why the ordering step matters.

**How to check it**

Look at the toolbar and count positions left to right against what the task asked for. For the scoping question, close and reopen the workbook: a button added **For all documents (default)** stays whatever file is open, a button scoped to the file appears only with that file. Reopening **File**, **Options**, **Quick Access Toolbar** and switching the **Customize Quick Access Toolbar:** list between the two entries shows which list the button actually landed in.

**Where it lands**

Week 2, session 4, as an addition. COBERTURA.md calls session 4 the tightest slot in the plan and writes the fallback in already: if something has to be dropped, drop this one and recover it in week 17, session 33, which also touches environment configuration. It is the only moment in the course when the interface itself is the subject.
...
Use the three small view buttons in the bottom right of the Status Bar, beside the zoom slider. Same three views, one click, no ribbon. They are what everyone uses. They do not reach **Custom Views...**, and a task phrased as "save the current print and filter setup as a view named Quarterly" has no Status Bar equivalent at all.

**Shortcut**

None assigned. `Alt`, `W`, `L` for Normal, `Alt`, `W`, `I` for Page Break Preview, `Alt`, `W`, `P` for Page Layout, by key tips.

**How to check it**

Look at the Status Bar view buttons: the active one is highlighted. In **Page Break Preview** the sheet is watermarked `Page 1`, `Page 2` and the breaks are visible as lines, so the view is unmistakable. For a custom view, reopen **Custom Views...** and confirm the name is in the **Views:** list. A workbook that contains an Excel table cannot store custom views at all and the **Custom Views** button greys out, which is worth showing the cohort before week 10 gives them tables.

**Where it lands**

Week 17, session 33, already in the syllabus: "workbook views, zoom, window, freeze panes, split". Exercise 4 covers page break preview, but COBERTURA.md flags that it never touches zoom, new window or arrange windows.
...
Use **Freeze Top Row** without selecting anything. It is right roughly half the time, because roughly half of real sheets have a one-row header. When the task asks for two header rows and a label column it is wrong, and it is wrong silently: the sheet still scrolls, one row still stays put, and nothing on screen announces the failure. The other short route is dragging a split bar and calling it frozen, which it is not, a split scrolls in both halves, a freeze does not.

**Shortcut**

`Alt`, `W`, `F`, `F` freezes at the selection. `Alt`, `W`, `F`, `R` freezes the top row. `Alt`, `W`, `F`, `C` freezes the first column. The same `Alt`, `W`, `F`, `F` unfreezes, because the first entry toggles.

**How to check it**

Look for the thin dark line where the freeze falls. It is a solid single line, unlike the thick grey bars a split draws. Then press `Ctrl+Down Arrow` and scroll: the frozen rows must stay on screen with the data moving beneath them. Reopen the **Freeze Panes** menu and the first entry must now read **Unfreeze Panes**, which is the cheapest proof that a freeze rather than a split is in effect.

**Where it lands**

Week 17, session 33, already in the syllabus, and practised earlier than that: Exercise 2 in week 3 already freezes panes alongside sheet management and named ranges.
...
Drag window borders by hand, or use the Windows key with the arrow keys to snap two Excel windows to half the screen each. It reaches the same visual arrangement and it is what people actually do. It does not create a second window onto one workbook, which is the whole point of **New Window**, and it never opens the **Arrange Windows** dialog where the four layouts and the **Windows of active workbook** check box live. For zoom, the slider at the bottom right corner is the short route: fast, and unable to hit an exact 85 per cent.

**Shortcut**

`Ctrl+F6` cycles open workbook windows. `Ctrl+W` closes the active window. Zoom answers to `Ctrl` plus the mouse wheel, which is again a short route with no exact value.

**How to check it**

Read the title bars. Two windows onto one workbook both carry the file name with `- 1` and `- 2` appended, and typing in one updates the other instantly, which is the test that separates **New Window** from opening the file twice. For zoom, read the percentage at the bottom right of the Status Bar. For a split, the thick grey bars are visible and both halves scroll independently.

**Where it lands**

Week 17, session 33, already in the syllabus: "window, freeze panes, split". COBERTURA.md counts 1.4.4 as covered but posts a material warning against it, Exercise 4 never touches zoom, new window or arrange windows, so this objective has a session but no prepared practice.
...
Type straight into the **Title** and **Tags** fields in the backstage panel without ever opening the dialog. It works for those two. It cannot reach Manager, Company or Hyperlink base without **Show All Properties**, and it cannot reach the **Custom** tab at all. The other short route is right-clicking the file in File Explorer and using its **Details** tab, which edits some of the same fields from outside Excel and is worth mentioning only so the cohort recognises it and does not use it in the exam.

**Shortcut**

`Alt+F` opens the backstage; `Alt+F`, `I` reaches Info directly. No shortcut opens **Advanced Properties**.

**How to check it**

Reopen **File**, **Info**. The panel reflects whatever the dialog wrote, so a Title set in the dialog appears in the panel and the other way round. Then go to **File**, **Info**, **Check for Issues**, **Inspect Document** and run it: the **Document Properties and Personal Information** section reports what is actually stored, which is both the proof that the properties are there and the bridge to 1.5.4. Note that the Author property is not one the exam usually leaves blank, and that **Last Modified By** cannot be typed at all.

**Where it lands**

Week 9, session 17, already in the syllabus: "inspecting workbooks, modifying basic properties, removing personal information".
...
Click a cell and read the Formula Bar, one cell at a time. That is the correct way to inspect a single formula and the wrong way to answer "show all formulas on the sheet", because it never changes the sheet state the exam is looking at. The third route is **File**, **Options**, **Advanced**, **Display options for this worksheet:**, and the **Show formulas in cells instead of their calculated results** check box, the same setting three panes deep.

**Shortcut**

`Ctrl+` grave accent, the key to the left of the `1` on most layouts. It toggles the same setting, and it is worth teaching because it is fast, reversible and easy to hit by accident: a student who triggers it mid-exam and does not know what happened will burn minutes.

**How to check it**

Look at the alignment. With **Show Formulas** on, everything including numbers goes left-aligned, cells containing constants show the constant, and cells containing formulas show text beginning with `=`. Verified on this machine: the stored `ColumnWidth` does not change when the toggle goes on (it stays at its set value while the display widens) so a student who thinks Show Formulas resized their columns permanently can be shown the number in **Format**, **Column Width...** as proof it did not.

**Where it lands**

Week 3, session 6, already in the syllabus, which names it literally: "formula auditing (show formulas)". COBERTURA.md notes that this single phrase is the only thing propping up four Expert auditing objectives, which is why weeks 3 and 7 both get auditing additions.
...
Select the range and print with **Print Selection** in the Print backstage. It prints the right cells once and stores nothing, so the next print goes back to the whole sheet. The other short route is typing the reference into the **Print area:** box on the **Sheet** tab of the **Page Setup** dialog (1.3.1), a legitimate second path to the same setting, and the one to use when the task gives an address rather than a range to select.

**Shortcut**

None. `Alt`, `P`, `R`, `S` sets it by key tips.

**How to check it**

Two checks, and the second is the one worth teaching. First, switch to the **View** tab and click **Page Break Preview**: everything outside the print area is greyed. Second, go to the **Formulas** tab, **Defined Names** group, and open **Name Manager**. Setting a print area creates a sheet-scoped defined name called `Print_Area`. Verified on this machine: after setting the area, Name Manager holds `Sheet1!Print_Area` referring to `=Sheet1!$A$1:$F$40`, and setting print titles adds `Sheet1!Print_Titles` referring to `=Sheet1!$1:$1`. If those names are absent, no print area was set no matter what got printed.

**Where it lands**

Week 17, session 33, already in the syllabus: "page setup, print area". Exercise 4 drops a twenty-page print job to eight pages by setting the area, which makes the effect visible rather than theoretical.
...
Press `F12`, type a file name, and change **Save as type:** in the Save As dialog. This reaches every format including PDF and it is what a fluent user does. It skips the **Export** pane, where **Change File Type** groups the formats with a description of what each one loses, and it skips **Options...** on the PDF publish, so "export the entire workbook to PDF" gets exported as the active sheet only. That single miss is the most common way this objective is failed.

**Shortcut**

`F12` opens the **Save As** dialog directly, bypassing the backstage. `Ctrl+Shift+S` reaches the Save As backstage pane. `Ctrl+S` on a file that has never been saved also opens Save As.

**How to check it**

Read the title bar: after a successful Save As it shows the new file name and extension, and the workbook now open in Excel is the new file, not the old one. For a `.csv` save, confirm that only one sheet remains addressable, because CSV holds a single sheet and Excel warns about it. For PDF, open the published file and count the pages against what **Publish what** was set to. For `.xltx`, confirm the file landed in the templates folder, since Excel redirects the Save As location as soon as the template type is picked.

**Where it lands**

Week 17, session 33, as an addition. COBERTURA.md argues the case: the session lead is "Preparing a workbook for printing and distribution", exporting is distributing, and the syllabus mentions .pdf and .docx only as submission rules rather than as taught content.
...
Add **Quick Print** to the Quick Access Toolbar and click it. It sends the active sheet to the default printer with whatever settings are already stored, no dialog, no preview. It is a genuinely useful button and it demonstrates nothing, because every setting the objective names is a setting it did not touch.

**Shortcut**

`Ctrl+P` opens the Print backstage. `Ctrl+F2` opens the same screen, since print preview and print settings are one pane in current Excel. On the backstage, `Alt` shows key tips for the settings lists.

**How to check it**

Read the page counter under the preview, `1 of 8` or similar, and step through with the arrows. Settings chosen here that also exist in **Page Setup** (orientation, margins, scaling) write back to the sheet, so reopening the **Page Setup** launcher shows them applied. Settings that live only in this pane (copies, collation, the printer) do not. That split is worth stating out loud, because students expect everything on this screen to be sticky and half of it is not.

**Where it lands**

Week 17, session 33, already in the syllabus. Exercise 4 is the practice file.
...
Delete comments by hand, unhide sheets by hand, and clear the Author field in the Properties panel. All three reach the same end state for the items you happened to notice, which is the problem: the objective is about finding what you did not notice. For compatibility, the short route is saving as `.xls` and reading the warning that appears at save time, which reports the same findings without ever opening the checker and without letting you choose which versions to test.

**Shortcut**

`Alt+F`, `I` reaches the Info pane. `Alt+R`, `A1` reaches Check Accessibility from the Review tab. None of the three dialogs has a Ctrl combination.

**How to check it**

Run **Inspect Document** a second time. Any category you removed must come back clean, and any you left alone must still report its count, which is the proof that only the named categories were touched. For the Accessibility Checker, the pane must read that no accessibility issues were found once the fixes are in. For the Compatibility Checker, click **Copy to New Sheet** and read the sheet it creates: a dated, itemised record that survives the dialog closing, and the only one of the three tools that leaves written evidence in the workbook.

**Where it lands**

Split across three weeks, which is worth flagging to the professor because no single session teaches the whole objective. The Document Inspector is week 9, session 17, already in the syllabus as "inspecting workbooks, modifying basic properties, removing personal information". The Accessibility Checker is week 11, session 22, already in the syllabus as "spelling, accessibility checker, comments". The Compatibility Checker is week 17, session 33, as an addition: COBERTURA.md calls it the missing third leg and puts it there precisely because the other two legs are already standing.
...
Copy, then click one of the small icons in the Paste gallery: the clipboard with **123** for values, the paintbrush for formatting, the two-arrow icon for transpose. Or press Ctrl+V and then click the **Paste Options** button that appears at the bottom right of the pasted block. Both reach Values and Transpose. Neither can reach **Operation** or **Skip blanks**, so half the objective is unreachable that way, and the item wording "using special paste options" points at the dialog.

**Shortcut**

**Ctrl+Alt+V** opens the Paste Special dialog directly with the copy already on the clipboard. Inside the dialog every option has an underlined accelerator: **V** for Values, **T** for Formats, **E** for Transpose. So Ctrl+Alt+V, V, Enter pastes values; Ctrl+Alt+V, E, Enter transposes.

**How to check it**

Click a pasted cell and read the formula bar. After Values it holds a constant, not a formula. After Paste Link it holds `=Sheet1!$A$1`. After Formats the destination keeps its own numbers and takes the source's look. After Transpose the block is turned and the first source row runs down the first destination column. If the source is still surrounded by the marching border, Esc was not pressed and the next keystroke may overwrite the clipboard.

**Where it lands**

Week 5, sessions 9 and 10. COBERTURA adds this one and states the reason plainly: the exam asks the candidate to choose an option, not to paste. Exercise 7 already practises pasting as values.
...
Drag the fill handle, the small square at the bottom right of the selection. Double-click it to fill down to the bottom of the neighbouring column. Right-drag it and pick **Fill Series**, **Fill Formatting Only**, **Fill Without Formatting**, **Fill Days** and so on from the menu that appears on release. Fast, and it cannot set a Stop value, cannot fit a trend, and cannot do Growth.

**Shortcut**

**Ctrl+D** fills down from the top row of the selection. **Ctrl+R** fills right from the leftmost column. Ribbon keys: Alt, H, F, I, S opens the Series dialog. (Expert 2.1.2 flags that same sequence as TO CONFIRM on the lab build; treat it as unverified until someone presses it.)

**How to check it**

The filled cells carry the pattern, not a repeated constant. The **Auto Fill Options** button appears at the bottom right of the filled block after a fill-handle drag and does not appear after Fill from the ribbon, which is the quickest visible tell of which route was taken. On a filled formula, the formula bar shows references shifted one row or one column per step.

**Where it lands**

Week 2, session 4, already in the syllabus as "AutoFill". Exercise 1 practises it.
...
Right-click the selected headings and click **Insert** or **Delete** on the context menu. The result is correct, but the item wording points at the ribbon and the context menu gives no chance to demonstrate the Insert Sheet Rows command by name. Worse, clicking the **face** of the Insert button rather than its arrow runs **Insert Cells** with a silent shift, which is objective 2.1.4 and can tear the grid apart when a partial range was selected.

**Shortcut**

**Ctrl+Shift+plus** inserts, **Ctrl+minus** deletes. With whole rows or columns selected both act immediately; with a partial range selected both open a dialog instead.

**How to check it**

The correct number of blank rows sits above the first row that was selected, and the row numbers have renumbered themselves. Formulas that pointed at the moved data still point at it, because Excel adjusted the references. After a delete, scan for `#REF!`: it means a formula was pointing into what was removed.

**Where it lands**

Week 3, session 5, already in the syllabus: "insert and delete columns and rows". Exercise 2 practises it, and Exercise 1 practises **Insert Sheet Columns** by name.
...
Click the face of the Insert button. It inserts and shifts down with no dialog and no choice offered, so if the task asked for Shift cells right it is already wrong. Right-clicking the selection and choosing **Insert...** opens the same dialog and gives the same result, so it is defensible on outcome, but it is not the ribbon path the item describes.

**Shortcut**

**Ctrl+Shift+plus** with a partial range selected opens the Insert dialog. **Ctrl+minus** opens the Delete dialog.

**How to check it**

The dialog must have appeared, and a shift direction must have been chosen in it. Afterwards the neighbouring data has moved in that one direction only and the rest of the sheet is untouched. If entire rows moved when only a block was selected, the wrong option was picked.

**Where it lands**

Week 2, session 4, already in the syllabus: "insert and delete cells". Exercise 1 practises **Delete Cells**.
...
Click the face of the Merge & Center button. One click, and it always does Merge & Center. When the task asks for Merge Across over five rows it produces one merged block instead of five and loses four rows of data.

**Shortcut**

No Ctrl combination. Ribbon keys: **Alt, H, M, C** for Merge & Center, **Alt, H, M, U** for Unmerge Cells.

**How to check it**

Click the merged area. The **Name Box** shows the top left address only, `A1` and not `A1:D1`, and **Merge & Center** shows as highlighted on the ribbon. For Merge Across, count the merged blocks: there must be one per row. Note that sorting and most filters refuse to run over merged cells, which is how the exam sometimes surfaces a wrong merge later in the same project.

**Where it lands**

Week 2, session 4, already in the syllabus: "merge cells". Exercise 1 practises Merge & Center.
...
The **Home** tab, **Alignment** group buttons: **Top Align**, **Middle Align**, **Bottom Align**, **Align Left**, **Center**, **Align Right**, the **Orientation** menu, **Increase Indent** and **Decrease Indent**. Every one is a separate operation. Three sub-skills, three clicks, three undo steps, and the Orientation menu offers only five preset angles, so any other angle is out of reach.

**Shortcut**

**Ctrl+1** opens Format Cells. Inside it, **Ctrl+Tab** and **Ctrl+Shift+Tab** move between tabs.

**How to check it**

Reselect the range and press Ctrl+1 again; the Alignment tab must show the values that were set. On the sheet, rotated headers render on the angle and the row height grows by itself, and indented text sits one character width further right per indent level. If **Increase Indent** is greyed out on the ribbon, Horizontal is set to Center, which is a common reason an indent silently fails to apply.

**Where it lands**

Week 2, session 4. COBERTURA adds this one, noting that the syllabus covers alignment and drops orientation and indentation, the other two sub-skills Certiport names in the same objective. Exercise 1 practises alignment.
...
Copy the source, then **Paste Special > Formats**. Same visible result, and it is objective 2.1.1, not this one. Or re-apply the font, fill and borders by hand on the destination, which also produces the same picture and demonstrates nothing.

**Shortcut**

No Ctrl combination in Excel. The Ctrl+Shift+C and Ctrl+Shift+V pair that copies formatting in Word is not bound here. Ribbon keys: **Alt, H, F, P**.

**How to check it**

The destination keeps its own values and takes on the source's font, size, colour, fill, borders, alignment and number format. Watch the pointer: after a single click the brush is gone from it, after a double-click it is still there, which is how you know the brush was locked before the second destination was painted. If numbers in the destination suddenly show as dates or percentages, the number format came across too, expected, and the reason Format Painter is not a safe habit outside the exam.

**Where it lands**

Week 2, session 4. COBERTURA adds this one, noting that the syllabus names the Clipboard group but not the brush. Exercise 1 copies the format of row 8 onto rows 9 to 12, which is the multi-target case.
...
Click **Wrap Text** on the **Home** tab, **Alignment** group. One click, one operation, and no way to set the vertical alignment in the same breath.

**Shortcut**

**Ctrl+1** for the dialog. Ribbon keys: **Alt, H, W** for the button. **Alt+Enter** while editing a cell inserts a manual line break, which looks like wrapping, is not wrapping, and is not accepted as such.

**How to check it**

The row height grows on its own to fit the wrapped lines, and the **Wrap Text** button shows as highlighted when the cell is selected. Then click the cell and read the formula bar: with Wrap text the string is one continuous line with no break in it. If the formula bar shows the text broken across two lines, someone pressed Alt+Enter instead and the check box is still clear.

**Where it lands**

Week 2, session 4. COBERTURA adds this one and explains why: the syllabus says "fit cells", which also reads as auto-fitting the column width, and that is objective 1.3.2. Exercise 1 practises Wrap Text.
...
Typing the format into the cell (`$1,200` or `50%`) and letting Excel infer it. A format does get applied, but Excel picks which one, so a task asking for Accounting gets Currency and the decimal count is whatever Excel decided. The nastier version: typing `50%` into a cell that already holds `50` turns the value into 0.5, so the underlying data is now wrong and every formula downstream is wrong with it.

**Shortcut**

On a US layout the number-row combinations are **Ctrl+Shift+1** two-decimal comma style (`!`), **Ctrl+Shift+2** time (`@`), **Ctrl+Shift+3** date (`#`), **Ctrl+Shift+4** currency (`$`), **Ctrl+Shift+5** percentage (`%`), **Ctrl+Shift+6** scientific (`^`), **Ctrl+Shift+~** general. Excel binds the symbol, not the digit, so on a Latin American keyboard the physical keys differ; check before teaching them as digits.

**How to check it**

Select a formatted cell. The **Number Format** list at the top of the Number group must show the name the task asked for. Then read the formula bar: it shows the raw value, unformatted, which proves the number was formatted and not retyped. Right alignment is a second tell, since numbers align right by default and text does not.

**Where it lands**

Week 2, session 4, already in the syllabus. Exercise 1 practises Increase Decimal. Custom number format codes belong to Expert 2.2.1 and COBERTURA sends them to week 4.
...
Go to the **Home** tab, **Font** group. Click the arrow next to **Font Color** and pick a colour. Click the arrow next to **Fill Color** and pick another. Click **Borders** and pick a preset. Three clicks, three separate operations, identical picture on screen. It is what a normal user does and what the student will do outside the exam, so learn it, and know that when the item says "from the Format Cells dialog box" it earns nothing.

**Shortcut**

**Ctrl+1** opens Format Cells on whichever tab was last used. **Ctrl+Shift+F** opens it on the **Font** tab. Inside the dialog, **Ctrl+Tab** moves to the next tab and **Ctrl+Shift+Tab** to the previous one, so the whole route runs without the mouse. **Enter** is OK. **Esc** is Cancel and throws away every tab.

**How to check it**

Reselect the range and press Ctrl+1 again, then walk the tabs: every setting must be showing where it was set. Then press **Ctrl+Z** exactly once. If the font colour and the fill both disappear together, the dialog did it and the route was the graded one. If only one of them disappears, the ribbon did it. Reapply and try again.

**Where it lands**

Week 2, session 4. COBERTURA adds this objective explicitly, budgets fifteen minutes for it, and gives the same reason this document gives: borders, fill and custom number formats live only in the dialog, and the exam splits 2.2.6 from 2.2.5 because of it. COBERTURA also flags session 4 as the tightest slot in the plan and says that if something has to be dropped it should be the Quick Access Toolbar, objective 1.4.1, not this. Exercise 1 already opens the dialog.
...
Reproducing the look by hand with font, size, colour and fill. Indistinguishable on screen, no style attached, so nothing is graded and the formatting will not follow a theme change. A second trap: **Format as Table** also makes cells look styled, and that is objective 3.1.2, a table style, not a cell style.

**Shortcut**

**Alt+'** (Alt plus the apostrophe key) opens the **Style** dialog directly. Ribbon keys: **Alt, H, J**.

**How to check it**

Select a styled cell and open **Home > Styles > Cell Styles**: the applied style is boxed in the gallery. A harder proof: go to the **Page Layout** tab, **Themes** group, and switch **Colors**. Cells carrying a themed style change colour; cells formatted by hand do not move.

**Where it lands**

Week 2, session 4, already in the syllabus: "styles (apply and clear)". Exercise 1 practises Cell Styles.
...
Pressing **Delete**. It runs Clear Contents, so the formatting survives and the objective is not met. Or applying the **Normal** cell style, which does reset font, fill, borders and number format and therefore looks like it worked, it is a style application, objective 2.2.7, and it leaves conditional formatting rules and data validation in place.

**Shortcut**

No dedicated key combination for Clear Formats. **Delete** clears the contents of the selection. **Backspace** clears the contents of the active cell only and drops into edit mode. Ribbon keys: **Alt, H, E, F**.

**How to check it**

The font goes back to the workbook default, Calibri 11 under the Office theme. The number format goes back to **General**, so a date turns into a five-digit serial number, the loudest single tell that Clear Formats really ran. Fills and borders vanish, the values are still there, and column width and row height do **not** change, because they are not cell formatting. If the values vanished too, **Clear All** was clicked by mistake.

**Where it lands**

Week 2, session 4, already in the syllabus: "styles (apply and clear)". Exercise 1 practises Clear All.
...
Select the range, click the **Name Box** at the left end of the formula bar, type the name and press Enter. It does create the name. It always scopes to **Workbook**, offers no **Comment**, and never shows the **Refers to** it built, so there is no chance to catch a wrong range. Any task that names a scope or a comment cannot be answered this way.

**Shortcut**

**Ctrl+F3** opens **Name Manager**. **F3** opens the **Paste Name** dialog while writing a formula, which drops a name into it. **Ctrl+Shift+F3** opens Create from Selection. Ribbon keys: **Alt, M, M, D** for Define Name.

**How to check it**

Open **Name Manager**: the name is listed with the right **Refers To**, the right **Scope**, and the comment if one was asked for. Then click the **Name Box** drop-down, the name appears there, and selecting it jumps to and selects the range, which proves the reference is live. A name whose **Refers To** reads `#REF!` is broken. COBERTURA warns that the workbook for Exercise 18 ships with six broken names left over from an old exercise, so students will meet this in week 11.

**Where it lands**

Week 3, session 6, already in the syllabus: "defined names (naming ranges)". Exercises 2 and 5 practise it, Exercise 5 using a defined name as an absolute reference. Navigating to a named range is a different objective, 1.2.2, which COBERTURA adds to the same session.
...
Leaving the table as `Table1`. Or renaming it in **Formulas > Name Manager**, where tables do appear with a table icon and can be renamed through **Edit...**. That works, and it is the Name Manager route, objective 2.3.1, not this one. Name Manager also refuses to let you change a table's **Refers to**, which is a good sign that it is not the tool the exam has in mind here.

**Shortcut**

None dedicated. The ribbon path is **Alt, J, T** for the **Table Design** tab, then the **Table Name** box. The KeyTip for the box itself is **TO CONFIRM**.

**How to check it**

Click inside the table, open **Table Design**, and read the **Table Name:** box. Better proof: any formula that already referenced the table rewrites itself, `=SUM(Table1[Amount])` becomes `=SUM(Sales[Amount])` with nothing typed. Open **Name Manager** and the table is listed with a table icon rather than a range icon. Start typing `=SUM(Sa` in a spare cell and the new name appears in the formula AutoComplete list.

**Where it lands**

Week 10, session 19, already in the syllabus: "name a table". Week 10 is the heaviest closing week in the plan. Exercise 16 builds a table named `Lorena` and writes its formulas in structured references, which is objective 4.1.2 and the reason naming the table matters at all.
...
Inserting a small line chart object and shrinking it over the cell. It looks close and it is a chart, which belongs to Domain 5, not here. It also sits on top of the grid instead of inside a cell, so it will not move, sort or print with the data.

**Shortcut**

None. Reach the ribbon with **Alt, N** and then the **Sparklines** group KeyTips, which are **TO CONFIRM**.

**How to check it**

Click the sparkline cell. The **formula bar is empty**, because a sparkline is a cell background and not a cell value, and the **Sparkline** contextual tab appears on the ribbon. That pair is the definitive test: a chart object would show selection handles and give you **Chart Design** instead. You can still type text into a sparkline cell and the sparkline stays behind it.

**Where it lands**

Week 5, session 9, already in the syllabus: "sparklines". Exercise 3 builds a line sparkline and a column sparkline.
...
Reading the numbers, then colouring the cells that meet the condition by hand with a fill. It is identical on screen the moment it is done, it takes about the same time on a short list, and it is not conditional formatting. This is the single most common way to lose points in this objective.

**Shortcut**

None. Ribbon keys: **Alt, H, L**.

**How to check it**

Change a value so it crosses the threshold, then change it back. A real rule repaints itself instantly; hand formatting does not move. Second check: **Home > Styles > Conditional Formatting > Manage Rules...**, then set **Show formatting rules for** to **This Worksheet**. The rule must be listed, with the right **Applies to** range and the right **Format** preview.

**Where it lands**

Week 5, session 9, already in the syllabus: "conditional formatting, icon sets". Exercise 3 applies highlight rules, top 10, bottom 10 per cent, above average, icon sets, a data bar and a colour scale. Writing your own rules from **New Rule...** is Expert 2.3.1 and formula-driven rules are Expert 2.3.2, both added to the same week by COBERTURA, which also notes that no exercise in the pack covers the formula-based case.
...
**Home > Editing > Clear > Clear Formats**. It does remove conditional formatting. It also removes every font, fill, border and number format on the selection, so anything the task wanted kept is gone with it. That is objective 2.2.8, and using it here usually breaks a later step of the same project.

**Shortcut**

None. Ribbon keys: **Alt, H, L**, then the **Clear Rules** submenu.

**How to check it**

Reopen **Manage Rules...** with **Show formatting rules for** set to **This Worksheet**. The deleted rule is gone from the list, the rules that were meant to survive are still there in the same order with **Stop If True** untouched, and on the sheet the cells keep the fills and fonts applied by hand and lose only what the rule was painting. If a whole column went plain white, Clear Formats was used instead.

**Where it lands**

Week 5, session 9. COBERTURA adds this objective and states the reason: Certiport counts it apart from 2.4.2, so clearing from the selected cells and clearing from the entire sheet are assessed separately.
...
Select the range with the mouse and click **Home** tab, **Styles** group, **Format as Table**, then pick a style. This creates the table and applies a style in one move. Fine when the task also names a style; it silently changes the formatting when the task only asked for a table, and the range Excel proposes is the same guess you never inspected.

**Shortcut**

`Ctrl+T` opens the **Create Table** dialog, and `Ctrl+L` does the same. Pressing `Ctrl+T` then `Enter` accepts Excel's guess without reading it, which is exactly the habit that loses points when the data touches a stray cell.

**How to check it**

Click inside the range: the **Table Design** tab must appear. Open **Formulas** tab, **Defined Names** group, **Name Manager**, and the table is listed there by name. Structured references become available, typing `=SUM(Table1[` offers the column names.

**Where it lands**

Week 10, session 19. COBERTURA.md lists 3.1.1 as covered by the syllabus line "apply table format, name a table". Practised in Exercise 16.
...
Fill the header row and shade alternate rows by hand from **Home** tab, **Font** group, **Fill Color**. The sheet looks the same. It is direct formatting, it does not follow when rows are inserted, and the exam checks the table style name, not the colours.

**Shortcut**

None. The gallery has no keyboard shortcut.

**How to check it**

Reopen the **Table Styles** gallery: the applied style is boxed. If the table still shows Blue, Table Style Medium 2, nothing was applied, because that is the default Excel gives on creation.

**Where it lands**

Week 10, session 19, covered by the syllabus.
...
Turn off the filter buttons, clear the banding check boxes, and call it done. The object is still a table, Name Manager still lists it, structured references still work. Nothing was converted.

**Shortcut**

None. The right-click equivalent is the same command: right-click a cell in the table, point to **Table**, click **Convert to Range**.

**How to check it**

Open **Formulas** tab, **Defined Names** group, **Name Manager**: the table name must be gone. Click a formula that used to read `Sales[Q1]` and confirm the formula bar now shows a cell address.

**Where it lands**

Week 10, session 19, covered by the syllabus line "convert a table to a range".
...
Type into the first empty row under the table, or into the first empty column beside it, and let auto expansion swallow it. This is verified behaviour: a formula typed in the column next to the table pulls that column into the table and fills the formula down the whole column. The other short route is dragging the small triangular sizing handle at the bottom right corner of the table. Both give the right table and demonstrate nothing.

**Shortcut**

None specific to table rows and columns. `Ctrl+Shift+plus` and `Ctrl+minus` act on worksheet rows and columns, which is the wrong object here.

**How to check it**

Open **Table Design** tab, **Properties** group, **Resize Table** and read the address. Or check that the banding still alternates correctly and that the total row is still the last row of the table.

**Where it lands**

Week 10, session 19, as an addition. COBERTURA.md flags this one explicitly: session 5 taught inserting rows and columns in the worksheet, and the table behaves differently.
...
Bold the first column from **Home** tab, **Font** group, and shade alternate rows by hand. Identical on screen. It breaks the moment a row is inserted, and the check boxes stay cleared, which is what gets read.

**Shortcut**

None for the individual check boxes.

**How to check it**

Look at the check boxes themselves in the **Table Style Options** group, then insert a row in the middle of the table and confirm the striping repairs itself.

**Where it lands**

Week 10, session 19, covered by the syllabus line "banded rows".
...
Type `=SUM(B2:B31)` in the first row under the table. The number is right. It is not a total row: the table does not own it, it does not react to a filter, and inserting a table row pushes it out of alignment.

**Shortcut**

`Ctrl+Shift+T` is reported to toggle the total row. **TO CONFIRM** on the lab machines before teaching it.

**How to check it**

Click a total cell and read the formula bar: it must start with `SUBTOTAL`, not `SUM`. Then filter the table on any column, a real total row recalculates on the visible rows only.

**Where it lands**

Week 10, session 19, covered by the syllabus line "total rows". Practised in Exercise 16.
...
Right-click a cell holding the value you want, point to **Filter**, and click **Filter by Selected Cell's Value**. The submenu also offers **Filter by Selected Cell's Color**, **Filter by Selected Cell's Font Color** and **Filter by Selected Cell's Icon**. One click, correct result, and it only exists when the criterion is already sitting in a cell.

**Shortcut**

`Ctrl+Shift+L` turns the filter arrows on and off. `Alt+Down Arrow` on a header cell opens that column's filter menu without the mouse.

**How to check it**

The filter arrow turns into a funnel icon on the filtered column, the row headings of the surviving rows turn blue, and the hidden row numbers are missing. The status bar at the bottom left reads how many of how many records were found.

**Where it lands**

Week 9, session 18, covered by the syllabus. Practised in Exercise 15, which filters by quarter, month, colour, text, icon and top ten.
...
Click in the column and use **Home** tab, **Editing** group, **Sort & Filter**, **Sort A to Z**, then repeat on the other column. Sorting twice in reverse priority order does reach the same arrangement in simple cases, and it leaves no sort levels behind for the grader to read.

**Shortcut**

None for the dialog. The single-column buttons live on the **Data** tab as **Sort A to Z** and **Sort Z to A**.

**How to check it**

Reopen the **Sort** dialog: the levels you defined are still listed, in order. On the sheet, find a tie in the first column and check that the second column decides it.

**Where it lands**

Week 9, session 17, covered by the syllabus line "sorting by multiple columns".
...
Type the dollar signs by hand. This one is honest: the exam grades the reference that ends up in the cell, not the key you pressed to get it. What loses points is the third habit, avoiding mixed references altogether by writing one formula per column. That produces the right numbers and fails a task worded "write one formula and fill it across the grid".

**Shortcut**

`F4` cycles the four forms; on laptops with a function lock, `Fn+F4`. `F4` pressed when no formula is being edited repeats the last action instead, which is a different command.

**How to check it**

Build a small multiplication grid: row headers across the top, column headers down the left. One formula with mixed references, `=$A2*B$1`, must fill the whole grid. If any cell shows a wrong product, a dollar sign is in the wrong place.

**Where it lands**

Week 3, session 6. The syllabus covers relative and absolute there. Mixed references are the addition COBERTURA.md flags, and it calls the gap expensive for exactly the multiplication-grid reason.
...
Drag over the cells with the mouse so the formula fills with `$B$2:$B$31`. The arithmetic is identical. When the task says "use the defined name" or "use a structured reference", the address is the wrong answer, and it also stops growing when the table gains rows.

**Shortcut**

`Ctrl+F3` opens **Name Manager**. `F3` opens **Paste Name** inside a formula. `Ctrl+Shift+F3` creates names from the selected labels.

**How to check it**

Read the formula bar: it must show the name, not an address. Open **Name Manager** and confirm the name and its scope. Press `F5` and the named ranges appear in the **Go To** list.

**Where it lands**

Split. The named-range half sits in week 3, session 6, and is used in formulas in week 4, session 7. The structured-reference half is the week 10, session 19 addition COBERTURA.md calls out, noting that session 19 names the table and never uses the name in a formula. Practised in Exercise 16, on the table called Lorena.
...
Type `=SUM(B2:B31)` straight into the cell, or read the total off the status bar at the bottom of the window. Both are right and neither shows the route.

**Shortcut**

`Alt+=` inserts SUM with a guessed range. `Shift+F3` opens **Insert Function** on an empty cell, and opens **Function Arguments** for the current function when the insertion point is already inside one.

**How to check it**

Select the range and read Sum, Average, Count, Min and Max on the status bar; if it does not show them, right-click the status bar and select them. Compare against the cell.

**Where it lands**

Week 4, session 7, covered by the syllabus.
...
Select the range and read **Count** or **Numerical Count** off the status bar. Correct, instant, and it leaves no formula in the workbook for the grader to find.

**Shortcut**

`Shift+F3` opens **Insert Function**, where typing "count" in **Search for a function** finds all three.

**How to check it**

Build the trap on purpose: put `=""` in one cell of the range and leave another truly empty. If COUNTA and COUNTBLANK together exceed the cell count, the formulas are right and the students have seen why.

**Where it lands**

Week 4, session 8. COUNTA and COUNTBLANK are in the syllabus; COUNT is the addition COBERTURA.md flags, on the grounds that the exam names it by function.
...
Type `=IF(B2>=70,"Pass","Fail")` by hand. Faster, and a quotation mark in the wrong place gives an error with no preview to catch it.

**Shortcut**

`Shift+F3` with the insertion point inside an existing IF reopens **Function Arguments** on that formula, which is the fastest way to repair a nested condition.

**How to check it**

Change one source value to sit exactly on the boundary. A `>=` test and a `>` test differ on that one row, and that is where the exam puts its check.

**Where it lands**

Week 5, session 10, again in depth in week 6, session 11, and week 7, session 14 for the nested form. Covered by the syllabus. Practised in Exercises 7, 8 and 12.
...
Type the first two or three results by hand and press `Ctrl+E` for Flash Fill. Excel infers the pattern and fills the column. The column is correct and completely static: change a source value and the split does not follow. When the task names the function, Flash Fill scores nothing. (Flash Fill in its own right is Expert 2.1.1.)
...
Edit one source cell and confirm the extracted value updates. Then check the last row of the column, where names of a different length usually expose an off-by-one in **Start_num**.

**Where it lands**

Week 4, session 8. LEFT is in the syllabus, recovered from the truncated PDF cell; RIGHT and MID are the additions COBERTURA.md flags as absent from all 34 sessions.
...
Retype the text in capitals, or count the characters by eye. Retyping breaks the link to the source and hides the trailing spaces that LEN was brought in to find.
...
Point LEN at a cell you believe is clean. A result one higher than the visible characters means a trailing space, which is the usual reason a lookup later fails.

**Where it lands**

Week 4, session 8. UPPER and LOWER are in the syllabus; LEN is the addition COBERTURA.md flags.
...
Write `=A2&" "&B2`. It concatenates correctly and it is not a function, so a task worded "use the CONCAT function" fails. The other trap is CONCATENATE, the retired predecessor, which still runs and is not what the objective names.
...
Put an empty cell in the middle of the range and compare the two results side by side. CONCAT closes the gap with nothing; TEXTJOIN with `FALSE` shows a doubled delimiter; TEXTJOIN with `TRUE` shows one clean separator.

**Where it lands**

Week 4. CONCAT is in the syllabus at session 7; TEXTJOIN is the addition COBERTURA.md flags, precisely because CONCAT has no delimiter and no way to skip empty cells.
...
Select the data and click one of the small gallery buttons in the **Charts** group, then pick a thumbnail. Or press `Alt+F1`. Both produce a chart. Neither lets you compare subtypes in the preview, which is what the task is testing when it names a specific one such as 100% Stacked.

**Shortcut**

`Alt+F1` creates a default chart on the same sheet. `F11` creates a chart on a new chart sheet.

**How to check it**

Click the chart border, then **Chart Design** tab, **Data** group, **Select Data**, and read the **Chart data range** box: it must hold the range the task named. Click **Change Chart Type** to confirm the subtype is highlighted.

**Where it lands**

Week 10, session 20, covered by the syllabus. Practised in Exercise 17, which builds twenty chart types across five data sheets.
...
Select the data and press `F11`. Excel builds a chart sheet immediately, using the default chart type and the name `Chart1`. It is the right kind of object with the wrong type and the wrong name, and there was no dialog in which to set either.

**Shortcut**

`F11` from the data. There is no shortcut for **Move Chart**.

**How to check it**

The chart sheet tab exists in the tab bar and shows no rows or columns when active. Right-click the tab and the sheet menu appears with **Rename** and **Move or Copy**. In the object model this is a `Chart` in `Workbook.Charts`, not a `ChartObject` on a worksheet, which is the distinction the exam is after.

**Where it lands**

Week 10, session 20, covered by the syllabus line "create chart sheets". COBERTURA.md warns this is a material gap: Exercise 17 builds twenty charts and never asks for a chart sheet, so this one needs a task written for it.
...
Click the chart so the source range shows its coloured outline on the sheet, then drag the sizing handle at the corner of the blue outline to swallow the adjacent column. It works only when the new column is next to the old ones, and it never touches the dialog the task is written against. Copying a range and pasting it onto the chart is the same kind of shortcut.
...
Reopen **Select Data Source** and count the entries under **Legend Entries (Series)**. Or click the new series on the chart and read the formula bar, which shows the SERIES formula, for example `=SERIES(Sheet1!$B$1,Sheet1!$A$2:$A$4,Sheet1!$B$2:$B$4,1)`. The last argument is the plot order.

**Where it lands**

Week 10, session 20, as an addition. COBERTURA.md notes it costs two minutes because session 20 already opens this same dialog to switch rows and columns.
...
Copy the source block, paste it elsewhere with **Paste Special** and **Transpose**, and build a second chart from the transposed copy. The picture matches, the workbook now carries a duplicate data block, and the original chart was never switched.
...
Read the category axis labels: they must now be what the legend used to hold. Count the series in the legend against the number of rows or columns in the source.

**Where it lands**

Week 10, session 20, covered by the syllabus line "switch rows and columns in the source data". COBERTURA.md flags it as a material gap: Exercise 17 never asks for it.
...
Use the **Chart Elements** button, the plus sign that floats at the top right corner of a selected chart. Select the check boxes for Chart Title, Data Labels, Legend and so on, and use the small arrow beside each for the position. Same set of elements. It gives no route to the ones that are not in the check box list, and the task usually names a position that is one level deeper.

**Shortcut**

None. `Esc` steps the selection back out to the chart area, which is worth teaching because most misfires come from having a series selected when you meant the chart.

**How to check it**

Go to the **Format** tab, **Current Selection** group, and open the **Chart Elements** list at the top left. It names every element in the chart, and selecting one there selects it on the chart. Use it to confirm the element exists and to reach elements too small to click.

**Where it lands**

Week 10, session 20, covered by the syllabus line "chart elements (title, axis titles, legend, data labels)".
...
Reproduce the arrangement element by element from **Add Chart Element**. The chart can be made to look the same. The layout number the grader reads is still unset.
...
Reopen **Quick Layout**: the applied layout is boxed in the gallery.

**Where it lands**

Week 10, session 20, covered by the syllabus line "quick layout".
...
Use the **Chart Styles** brush button that floats at the right edge of a selected chart, with its **Style** and **Color** tabs. It reaches the same galleries. Recolouring the series one by one with **Shape Fill** is the other short route, and that one leaves direct formatting that no style can then override.
...
Reopen the **Chart Styles** gallery: the applied style is boxed. In the object model the chart carries a numeric `ChartStyle`, verified writable, which is what changes when a style is applied and what does not change when you recolour by hand.

**Where it lands**

Week 10, session 20, covered by the syllabus line "styles, colours".
...
Type a caption in the cell under the chart, or put the explanation in the chart title. A screen reader announces the chart as an unlabelled object either way, and the alt text property stays empty, which is what the grader reads.
...
Run **Check Accessibility** again: the chart must no longer appear under **Missing alternative text**. Reopening the **Alt Text** pane shows the stored text. In the object model this writes the shape's `AlternativeText`, and **Mark as decorative** writes `Decorative`, both verified present on this build.

**Where it lands**

Week 10, session 20, covered by the syllabus line "alternative text". COBERTURA.md flags it as a material gap alongside chart sheets and switching rows and columns: Exercise 17 asks for none of the three, so this objective needs a task written for it.
...
Open the source module, select all the code, Ctrl+C, then in the destination press Alt+F11, **Insert** menu, **Module**, Ctrl+V. The Sub ends up in the right place and runs. The exam asks you to copy the macro, and the copy it can see is a module that arrived as a module, so the pasted-text route leaves the graded action unperformed.

**Shortcut**

Alt+F11 opens and closes the Visual Basic Editor. Ctrl+R shows the Project Explorer inside it. Alt+F8 opens the Macro dialog back in Excel. Alt+Q closes the editor and returns to the sheet.

**How to check it**

In the destination workbook press Alt+F8 and set **Macros in** to **This Workbook**: the macro name is listed and **Run** is available. In the editor, the destination project has a Modules folder containing the module. The saved file name ends in `.xlsm`.

**Where it lands**

Nowhere in the 34 sessions. COBERTURA.md files this under "Lo que no cabe", section 1, and proposes a 90-minute workshop in the independent-work window of week 15, built from the TIA503 `w01` deck. It also warns that the Personal Macro Workbook and dragging a module between projects are the half of this objective TIA503 never touches, so that half needs new material.
...
Type the whole thing by hand, path, brackets, quotes and all. One wrong character and the formula returns `#REF!` and the exam sees a broken link. Worse, copying the value across instead of linking gives the right number today and a stale number tomorrow, and there is nothing for Edit Links to list.

**Shortcut**

Ctrl+Tab moves between open workbook windows while you are pointing. Ctrl+grave accent toggles **Show Formulas** so every external reference on the sheet is visible at once. Ctrl+F3 opens Name Manager to inspect a name that points outside the file.

**How to check it**

Click the cell and read the formula bar. An external reference always carries square brackets around the file name and a `!` after the sheet name; close the source workbook and the path appears inside single quotes. **Data** tab, **Edit Links** lists the source with **Status** reading OK after **Check Status**. **Formulas** tab, **Trace Precedents** draws a dashed arrow ending in a small worksheet icon, which is the arrow Excel reserves for a reference in another workbook.

**Where it lands**

Week 13, sessions 25 and 26. COBERTURA.md adds it there because the guided practice consolidates across sheets of one workbook, while the objective asks for links across files.
...
Click **Enable Content** on the yellow bar and carry on. It runs the macros and demonstrates nothing about the setting that produced the bar. The exam usually asks for the state of Macro Settings or for a trusted location, neither of which the yellow bar touches.

**Shortcut**

Alt+F, T opens Excel Options through the backstage. There is no shortcut into the Trust Center. Alt+F8 shows whether the macros are live: if they are still disabled the list is empty.

**How to check it**

Press Alt+F8, the macro names are listed and Run is available. Go to the **File** tab, **Info**, and there is no blocked-content notice. Reopen **Developer**, **Macro Security** and the option you set is still selected.

**Where it lands**

Nowhere in the 34 sessions. COBERTURA.md sends it to the macro workshop of "Lo que no cabe", section 1, and notes that the TIA503 `w01` slide "Cuatro pasos para dejar el entorno listo" already covers this objective end to end: Developer tab, save as `.xlsm`, enable on open, Alt+F11.
...
Reopen the file and wait to see whether the Document Recovery pane offers something on the left. It sometimes does. It is not a route, it is luck, and it disappears the moment you dismiss the pane.

**Shortcut**

Alt+F opens the backstage, then I selects Info. Ctrl+S is the reason versions exist and the fastest way to stop needing them.

**How to check it**

**File** tab, **Info** lists versions with time stamps under **Manage Workbook**. **File** tab, **Options**, **Save** shows the minute interval you typed and the AutoRecover path. After a Restore, the title bar file name is unchanged and the cell contents are the older ones.

**Where it lands**

Week 9, sessions 17 and 18. COBERTURA.md puts it there because the session already opens File, Info to inspect the workbook and edit properties, so this is five more minutes in a pane that is already on screen.
...
Set the file to read-only in Windows, through its Properties in File Explorer. Excel will honour it and the workbook is protected in the plain sense. Nothing inside the file changed, so File, Info still shows an unprotected workbook and the grader finds nothing.

**Shortcut**

None. Alt+F, I reaches the Info pane, which is the entry point for every branch above.

**How to check it**

Close and reopen. A workbook marked as final opens with a yellow MARKED AS FINAL bar carrying an **Edit Anyway** button, the ribbon collapsed and typing blocked. An encrypted workbook asks for the password before it opens. Always Open Read-Only shows the message that the author would like you to open this as read-only. In every case **File** tab, **Info** paints the Protect Workbook tile yellow and writes underneath it what the restriction is.

**Where it lands**

Week 11, session 22. COBERTURA.md adds it there: the syllabus already protects the sheet and the workbook in that session, and this objective is the third branch of the same backstage menu.
...
Go to the **Review** tab, click **Protect Sheet**, and click **OK**. Every cell in the sheet was locked to begin with, so the whole sheet freezes and nobody can type anywhere. The task asked for a sheet where some cells are still editable, and this route produces one where none are.

**Shortcut**

Ctrl+1 opens Format Cells, and it is the only keyboard shortcut in this route. Protect Sheet and Allow Edit Ranges have none; they are reached through the Review tab.

**How to check it**

Click a locked cell and type: Excel refuses with the message that the cell or chart you are trying to change is on a protected sheet. Click a cell inside the named range and type: Excel asks for the range password instead of refusing outright, which is the proof that Allow Edit Ranges took. Select a formula cell that had **Hidden** set and the formula bar stays empty while the cell still shows its result. The Review tab button now reads **Unprotect Sheet**.

**Where it lands**

Week 11, session 22. The syllabus already protects the worksheet and the workbook there; COBERTURA.md adds the range half, because the ranges are the part the syllabus never names. Exercise 18 is the file for it: age calculated in column J, that formula hidden and protected, the salary cells protected, then the sheet protected.
...
Right-click each sheet tab and click **Hide**. The sheets disappear and anyone can bring them back with **Unhide** in three clicks. The workbook is not protected, it is tidied.
...
Right-click any sheet tab: **Insert**, **Delete**, **Rename**, **Move or Copy**, **Tab Color**, **Hide** and **Unhide** are all dimmed, while **View Code** and **Protect Sheet** stay available. **File** tab, **Info** paints the Protect Workbook tile yellow and writes that the structure of the workbook has been locked. Try to drag a sheet tab and the cursor refuses.

**Where it lands**

Week 11, session 22, already covered by the syllabus, which names protecting the worksheet and the workbook in that session.
...
Leave everything on Automatic, where it already is, and never open the menu. The workbook is correct and the objective is untouched. It also guarantees trouble in week 16, because a two-variable data table recalculates on every keystroke unless the mode is Automatic Except for Data Tables.

**Shortcut**

F9 recalculates all open workbooks. Shift+F9 recalculates the active sheet. Ctrl+Alt+F9 forces a full recalculation of every formula whether or not Excel thinks it changed. Ctrl+Shift+Alt+F9 rebuilds the dependency tree first and then does that.

**How to check it**

With Manual set, change a precedent and the word Calculate appears on the status bar while the result stands still; press F9 and both catch up. **File** tab, **Options**, **Formulas** shows the radio button matching the ribbon menu. With iterative calculation on, a deliberate circular reference stops raising the circular reference warning and settles on a number instead.

**Where it lands**

Week 3, session 6, the session that opens the Formulas tab. COBERTURA.md gives it five minutes there and flags that it comes back in week 16, because data tables depend on the calculation mode.
...
Right-click the cell and click **New Note**, then type. It looks like a comment, it prints like a comment, and it is a different object. If the task says comment, a note scores nothing, and the reverse is also true.

**Shortcut**

Shift+F2 inserts or edits a comment on the selected cell. Ctrl+Shift+F2 inserts a note on builds that carry both, **TO CONFIRM** on the lab build. Ctrl+Enter posts the comment you have just typed.

**How to check it**

A threaded comment marks the cell with a small purple triangle in the top right corner and its box carries a **Reply** field. A note marks the cell with a red triangle and its box is a plain yellow rectangle with no Reply field. **Review** tab, **Show Comments** lists threads only; a note never appears in that pane.

**Where it lands**

Week 11, session 22, already covered by the syllabus line on comments. COBERTURA.md adds one nuance for the Microsoft 365 track: comments moved down from the Expert to the Associate in MO-210, and the threaded comment against note distinction is graded there and did not exist in 2019.
...
Change the Windows display language in Settings and let Office follow. It often does. It changes nothing inside Excel Options, so the pane the grader looks at is untouched.

**Shortcut**

Alt+F, T opens Excel Options. There is no shortcut to the Language pane itself.

**How to check it**

Reopen **File** tab, **Options**, **Language**. The preferred display language sits at the top of its list with the word Preferred beside it. Restart Excel and the ribbon tab names are in the new language. In the authoring list, the **Proofing** column tells you whether spelling will actually work in that language.

**Where it lands**

Nowhere in the 34 sessions. COBERTURA.md puts 1.3.1 and 1.3.2 in "Lo que no cabe", section 2, anchored to the INNOVATIQ self-paced component that runs all semester and is worth 5 per cent, and notes that MO-211 deletes the language domain outright, so this objective only exists if the cohort sits MO-201.
...
Type the Spanish month name as text and format the cell as General. It reads correctly and it is not a date any more, so it will not sort, will not filter by month and will not feed a date grouping in a PivotTable.

**Shortcut**

F7 opens Spelling. Ctrl+1 opens Format Cells for the locale work. There is no keyboard shortcut for Translate in Excel.

**How to check it**

Press Ctrl+1 on the formatted cell and read the **Custom** category: the format code starts with a bracketed locale id such as `[$-80A]` or `[$-409]`. The formula bar still shows a serial date, not text, which proves the cell was formatted and not retyped. Right alignment happens by itself, because Excel right-aligns numbers and left-aligns text.

**Where it lands**

Nowhere in the 34 sessions. Same as 1.3.1: "Lo que no cabe", section 2, anchored to the INNOVATIQ self-paced course, and gone entirely from MO-211.
...
Write `=PROPER(LEFT(A2,FIND(" ",A2)-1))` or a chain of `LEFT`, `MID`, `FIND` and `TEXTJOIN`, drag it down, then paste as values. The column ends up identical. It is a different objective, it belongs to domain 3, and when the task says fill the column based on the pattern in the cells above, the formula earns nothing here.

**Shortcut**

Ctrl+E fills the column by pattern from wherever the cursor is. It is the whole objective in one key and it is the one the exam expects you to know.

**How to check it**

Click any filled cell: the formula bar shows a constant, not a formula. That is the fastest way to tell a Flash Fill from a formula solution. The Flash Fill Options button sits beside the range until you type somewhere else.

**Where it lands**

Week 9, sessions 17 and 18, in the part of the session that separates and cleans columns. COBERTURA.md is explicit that this is not the AutoFill of week 2: AutoFill continues a series, Flash Fill infers a pattern, and the two are separate objectives.
...
Type the first two values, select both, drag the fill handle. Excel infers the step and fills. It handles a linear step and nothing else: no growth, no weekday-only calendar, no stop value, no trend. When the task asks for a series that doubles or that skips weekends, the drag cannot produce it.

**Shortcut**

Ctrl+D fills down from the cell above, Ctrl+R fills right from the cell to the left. Neither opens the Series dialog. The ribbon key tips reach it as Alt, H, F, I, S, **TO CONFIRM** that sequence on the lab build.

**How to check it**

Click a cell in the middle of the series: the formula bar shows a constant, never a formula, which separates a filled series from a dragged formula. In a Growth series the ratio between neighbours is the step. In a Weekday series no Saturday or Sunday appears, so a `WEEKDAY` check on the column returns only 2 to 6.

**Where it lands**

Week 3, session 5. COBERTURA.md puts it there because that session already copies and moves sheets, and because week 2 taught the drag while this is the session that opens the box behind it.
...
**Home** tab, **Number** group: **Increase Decimal**, **Comma Style**, **Percent Style**. Three clicks and the column looks tidy. Those buttons cannot append a unit, cannot colour negatives, cannot suppress zeros and cannot scale to thousands, so the moment the task names any of those the ribbon route runs out.

**Shortcut**

Ctrl+1 opens Format Cells and lands on whichever tab was last used, so press it once to check. The direct format shortcuts listed at Associate 2.2.5 all apply a built-in format and none of them reaches Custom.

**How to check it**

Select the cell and press Ctrl+1. The **Category** list shows **Custom** selected with your code in the **Type** box. The formula bar shows the original number untouched, which proves you formatted the value rather than retyping it. Change the underlying number to a negative and then to zero, and watch the second and third sections take over.

**Where it lands**

Week 4, sessions 7 and 8, next to the text and date functions. Exercise 13 already applies custom number formats over 60 students, so half of this is practised outside class.
...
Type the allowed values into a spare column and tell people to copy from it. Or add a conditional format that turns bad entries red. Both communicate the rule and neither stops anyone typing whatever they like, so nothing is validated and nothing is graded.

**Shortcut**

Alt+Down Arrow opens the in-cell dropdown once the rule exists, which is the shortcut the exam actually rewards when it asks you to enter a value from the list. The dialog itself is reached with the key tips Alt, A, V, V, **TO CONFIRM** that sequence on the lab build.

**How to check it**

Select a validated cell: the dropdown arrow appears if In-cell dropdown was kept, the input message balloon appears if the Input Message tab was filled, and typing an invalid value raises the titled box with the icon that matches the Style you chose. Go to **Home** tab, **Editing** group, **Find & Select**, **Data Validation** to select every cell carrying any rule; **Find & Select**, **Go To Special...**, **Data validation**, **Same** selects only the cells sharing the active cell's rule.

**Where it lands**

Week 9, sessions 17 and 18. COBERTURA.md notes that the word validation appears zero times in the syllabus and that this belongs in the Data tab session that is already open. Exercise 14 already carries all five rule types including the custom formula that caps the bonus at 10 per cent of salary, so the class time is fifteen minutes and the practice is done at home.
...
Select the rows, right-click a row header and click **Hide**. The screen looks the same as a collapsed group. There is no outline bar, no level buttons, and nothing that says these rows belong together, so there is nothing for the exam to find.

**Shortcut**

Alt+Shift+Right Arrow groups the selection. Alt+Shift+Left Arrow ungroups it. Ctrl+8 shows and hides the outline symbols, which is also the fastest proof that an outline exists at all.

**How to check it**

The outline bar sits in a margin to the left of the row headers, with numbered level buttons above it. Press Ctrl+8: the bar vanishes and returns, which hidden rows never do. Collapse a group, select the range and press Alt+; to select visible cells only, the collapsed rows are excluded, exactly as they would be under a filter.

**Where it lands**

Week 14, sessions 27 and 28. COBERTURA.md adds it there and points out the reason: the subtotals in that same session already build an outline and nobody explains where it came from.
...
Type `=SUM()` under each group by hand. The arithmetic is right. There is no outline, the grand total double-counts the subtotals unless you dodge them by hand, and inserting one row breaks every range you wrote.

**Shortcut**

Alt+= inserts AutoSum, which writes `SUM` and not `SUBTOTAL`. Ctrl+Shift+T is reported to toggle the Total Row on a table, **TO CONFIRM** on the lab build.

**How to check it**

Click a subtotal cell and read the formula bar: it must say `=SUBTOTAL(9,...)` and not `=SUM(...)`. The outline bar shows three levels; level 2 shows only the subtotal rows, level 1 only the grand total. Apply a filter and the number moves if the function code is `109` and stands still if it is `9`.

**Where it lands**

Week 14, sessions 27 and 28, already covered by the syllabus. Exercise 22 is the file: 576 rows of Mexico City population data across four copies of the sheet, a different subtotal on each.
...
Sort the column and delete the repeats by eye, or write a `COUNTIF` helper column and filter it. Both reach the same list. Neither is the command the objective names, and the helper column leaves a column behind that the task never asked for.

**Shortcut**

None. The key tips reach it as Alt, A, M, **TO CONFIRM** on the lab build.

**How to check it**

Select the data block and read the count on the status bar: it dropped by exactly the number the message reported. Then apply **Conditional Formatting**, **Highlight Cells Rules**, **Duplicate Values** to the same range, nothing highlights.

**Where it lands**

Week 9, sessions 17 and 18, already covered by the syllabus line on removing duplicates. Exercise 14 does it on `Students_data.csv`, seven columns.
...
**Conditional Formatting**, **Highlight Cells Rules**, **Greater Than...**, type the number, take **Light Red Fill with Dark Red Text** from the **with** list. That does apply a conditional format, and it is the correct answer when the task names a preset (Associate 2.4.2). When the task describes a threshold and a format that is not in the preset list, the preset route cannot express it.

**Shortcut**

None for New Rule. Note that Ctrl+1 does not work inside the rule dialog; the **Format...** button is the only way to the format from there.

**How to check it**

Go to **Home** tab, **Styles** group, **Conditional Formatting**, **Manage Rules...**, and set **Show formatting rules for** to **This Worksheet**. The rule is listed with a preview in the **Format** column, its range in **Applies to** and its **Stop If True** check box. Then select a formatted cell and press Ctrl+1: the cell's own Font and Fill are unchanged, which proves the colour is coming from the rule and not from direct formatting.

**Where it lands**

Week 5, sessions 9 and 10. COBERTURA.md adds it there: the syllabus session already covers conditional formatting and icon sets from the preset menus, and this is the same menu one entry lower.
...
Add a helper column with `=IF($E2="Overdue","X","")` and apply **Highlight Cells Rules**, **Text that Contains** to the helper. It highlights the helper column and nothing else, and it adds a column the task never asked for.

**Shortcut**

None for the rule. F4 does cycle a reference between relative, absolute and the two mixed forms while the cursor sits on it inside the formula box, and that is the key you use to get from `E2` to `$E2` without retyping.

**How to check it**

Open **Manage Rules...** and read the **Rule (applied in order shown)** column: it reads `Formula: =...` for this rule type. Then copy the formula out of the box, paste it into an empty cell beside the top left cell of the range, and drag it down: it must return TRUE on exactly the rows that are coloured and FALSE on the rest. If it returns TRUE everywhere, a dollar sign is missing or the wrong cell was active when the rule was written.

**Where it lands**

Week 5, sessions 9 and 10. COBERTURA.md singles this objective out: it is what separates the Expert from the Associate at this point, none of the 25 exercises does it, and it is one of the few places where new material has to be written rather than assigned.
...
Select the range, clear all the conditional formatting, and build the rules again in the order you want. It works, and it throws away every rule that was already correct, including the ones outside the selection that you did not know were there.
...
The manager lists the rules top to bottom in priority order, with a live preview in the **Format** column and the range in **Applies to**. Turn Stop If True on for the top rule and the lower rule stops showing on the cells the top rule matched, while staying in the list, which is the difference between suppressing a rule and deleting it. Set **Show formatting rules for** to **This Worksheet** before deciding that a rule is gone.

**Where it lands**

Week 5, sessions 9 and 10. COBERTURA.md adds the manager there as the fourth of the five additions that all come out of the same dialog, which is why the week absorbs them.
...
Type the formula straight into the cell and let Formula AutoComplete finish the name, then read the yellow ScreenTip under the cell to know which argument you are on. It is faster, every working analyst does it, and it earns nothing extra because the exam scores the formula that ends up in the cell, not the typing. The reason to still teach the dialog is the nested case: a student who types a three-level IFS with no dialog gets the parentheses wrong and loses the whole item, and the dialog is the only route where Excel counts the parentheses for you.

**Shortcut**

Shift+F3 opens **Insert Function**, from an empty cell or from inside a formula. F4 while a reference is selected cycles A1, $A$1, A$1, $A1. Ctrl+A right after typing a function name and its opening parenthesis opens the Function Arguments dialog for that function.

**How to check it**

Select the cell and read the formula bar. The nest must sit inside the argument, not be pasted next to it: `=IF(AND(B2>=10,C2="Yes"),"Bonus","No")`, not two formulas in two cells. Then click into the cell and press Shift+F3 again. If the formula was built cleanly, Function Arguments reopens on the outer function with every box filled. If it opens with the whole formula sitting inside the first box, the nest was typed as text and will fail a grader that re-parses it.

**Where it lands**

Split across two weeks by COBERTURA.md. Week 7, sessions 13 and 14, takes NOT, IFS and SWITCH as additions to the AND, OR and nested IF the syllabus already schedules there, with Exercises 8, 10 and 12. Week 8, sessions 15 and 16, takes the singular SUMIF, COUNTIF and AVERAGEIF as an addition, ahead of the plural family the syllabus schedules, with Exercises 9 and 11. IF itself is already syllabus content in weeks 5 and 6. Material gap flagged in COBERTURA.md: Exercise 11 practises only SUMIFS, COUNTIFS and AVERAGEIFS, so MAXIFS and MINIFS have no exercise and week 8 has the slack to add them.
...
Type `=VLOOKUP(` and drag the table with the mouse, then hit F4. Or, on this machine, skip the objective's four functions and write `=XLOOKUP(...)`, which needs no column number and defaults to exact match. XLOOKUP exists here and returns without error, but MO-201 asks for these four by name: a project that says "use a function that returns the price for the code in column A" is marked on the result and XLOOKUP would pass, while leaving the student unable to answer the item that says "use the function that finds a value in the leftmost column". Teach XLOOKUP as the modern habit and the four as the exam vocabulary.

**Shortcut**

Shift+F3 for Insert Function. F4 on the Table_array reference. Ctrl+Shift+Enter is not needed for any of these in Microsoft 365.

**How to check it**

Read the formula bar and check three things: the table array carries dollar signs; Range_lookup is written out as FALSE, not left blank; and for the INDEX plus MATCH pair, MATCH sits inside INDEX's Row_num, so the formula reads `=INDEX($C$2:$C$50,MATCH($A2,$B$2:$B$50,0))` and not two separate cells wired together. Then fill the formula down two rows and confirm the answers still track the right table.

**Where it lands**

Week 13, sessions 25 and 26. The syllabus already schedules VLOOKUP for session 24 and VLOOKUP plus HLOOKUP for sessions 25 and 26; INDEX and MATCH are the addition, twenty minutes. Exercises 20 and 21. COBERTURA.md notes that session 26's syllabus cell is cut off in the PDF at "VLOOKUP, HLOOKUP and", so INDEX and MATCH may already have been intended there.
...
Ctrl+semicolon stamps today's date into the cell, Ctrl+Shift+semicolon stamps the current time. They look identical on screen the day you type them and they are not the same thing at all: they write a fixed value, not a formula, so they never change again. The exam item that says "insert a value that shows the current date every time the workbook is opened" is failed by the keystroke and passed by the function. Show both side by side, close the file, reopen it the next class, and let the students see one cell move and the other not.

**Shortcut**

Ctrl+semicolon for a static date. Ctrl+Shift+semicolon for a static time. F9 forces a recalculation, so the volatility of NOW can be demonstrated without waiting.

**How to check it**

Click the cell and read the formula bar: it must show `=TODAY()` or `=NOW()`, with the empty parentheses. A cell showing today's date with a bare date in the formula bar was done the ungraded way. Then press F9 and watch a NOW cell move its seconds.

**Where it lands**

Week 4, sessions 7 and 8, alongside the DAY, MONTH and YEAR the syllabus already schedules in session 8. Exercise 13 supplies the file, sixty students with birth dates.
...
Add days with plain arithmetic, `=B2+5`, and eyeball whether the answer fell on a weekend. It is what people do and it is wrong every time the range crosses a Saturday. There is also WORKDAY.INTL, which takes a weekend pattern for countries that do not rest on Saturday and Sunday, and it exists on this machine. It is not in the objective, so it is worth one sentence and no more.

**Shortcut**

None specific to these two. Shift+F3 to reach the dialog.

**How to check it**

Feed WORKDAY a start date of a Friday and a Days of 1 and confirm it returns the following Monday, not Saturday. Then add that Monday to the holidays range and confirm the answer moves to Tuesday. If it does not move, the Holidays argument was left empty and the route was incomplete.

**Where it lands**

Nowhere in the 34 sessions. COBERTURA.md puts this in "Lo que no cabe", item 4: week 4 already absorbs six added functions on top of the ten the syllabus schedules for session 8, and two more that need the holidays argument explained would break it. The proposal there is a take-home with a commented solution, built on Exercise 13's file, which already carries sixty birth dates and already practises DAY, MONTH and YEAR. Fifteen minutes of student time, no class time.
...
Write `=Sheet1!B2+Sheet2!B2+Sheet3!B2`, or use a 3-D reference, `=SUM(Sheet1:Sheet3!B2)`. Both work, both are what an experienced user reaches for, and both add by cell position. They give the wrong total the moment one sheet lists its regions in a different order, which is the exact case the exam builds. Consolidate with **Top row** and **Left column** ticked adds by label. Say this out loud in class; it is the whole point of the feature.

**Shortcut**

Alt then A then N is the classic key sequence for this command and still works. There is no single-key shortcut.

**How to check it**

Two artifacts prove the graded route. Reopen **Consolidate** on the same target cell: the **All references:** list comes back populated, because Excel stores the definition with the sheet. And if **Create links to source data** was ticked, outline level buttons 1 and 2 appear at the top left of the row headers, and expanding a group shows one row per source with an external link formula. A summary built by typing shows neither.

**Where it lands**

Week 13, sessions 25 and 26, ten minutes. It is an addition because CLO08 says the course "consolidates information with lookup functions, subtotals, PivotTables and what-if analysis tools" and names every method except the one actually called Consolidate.
...
For Goal Seek: type numbers into the input cell and watch the result until it looks right. It converges eventually and it leaves no trace that any tool was used. For Scenario Manager: copy the sheet three times and change the inputs on each copy. Both are what students do when nobody is watching. Neither produces the artifact the grader looks for, and the copy-the-sheet habit is exactly the mess Scenario Manager exists to prevent.

**Shortcut**

None for either dialog. Alt then A then W opens the What-If Analysis menu, then G for Goal Seek and S for Scenario Manager.

**How to check it**

For Goal Seek, the input cell now holds a number that was not typed and the target cell shows the value asked for, usually with a long tail of decimals because Goal Seek stops at a tolerance rather than landing exactly. For Scenario Manager, reopen it: the named scenarios sit in the **Scenarios:** list with their **Changing cells:** and **Comment:** shown below. If a Summary was asked for, a sheet named Scenario Summary exists with a grouped outline down the left. Three copies of the sheet are not a scenario.

**Where it lands**

Week 16, sessions 31 and 32, counted by COBERTURA.md as already covered by the syllabus line "Goal Seek, data tables and scenario analysis". One warning: Exercise 25 describes Scenario Manager and never actually uses it in any of its three cases, so the Scenario Manager half of this objective has no practice file today, and week 16 is already carrying project presentations. Fifteen minutes have to be budgeted for it.
...
Build a column of running balances and count the rows until it reaches zero. It is a legitimate model and it is often clearer to a reader, but it is not a function and the item asks for one. The other short route is typing `=NPER(B2/12,-B3,B4)` straight in, which is fine once the student already knows the argument order and is a trap before that, because Rate, Pmt, Pv reads naturally while PMT's own order is Rate, Nper, Pv and the two get swapped constantly.

**Shortcut**

Shift+F3 for Insert Function. Ctrl+A after typing `=NPER(` opens the same dialog.

**How to check it**

The NPER cell returns a positive number of periods. If it returns a negative number or `#NUM!`, Pmt and Pv carry the same sign and the model is wrong even though the formula is spelled right. For the decision cell, read the formula bar: AND must sit inside IF's first argument, `=IF(AND(D2<=36,C2<=5000),"Approve","Reject")`. Two separate cells, one holding AND and one holding IF, is a different formula and will not match.

**Where it lands**

Week 16, sessions 31 and 32, as an addition. AND and IF are already taught in weeks 5 to 7, so what is new here is NPER, and COBERTURA.md budgets it together with PMT at fifteen minutes for the pair on a single credit model. Exercise 25 provides the credit case, though only in .docx, with no workbook.
...
Type `=PMT(0.12/12,24,-100000)` with the numbers written into the formula. It returns the right payment and it fails any item that says "so that the payment updates when the rate changes", because there is nothing to update. It also hides the sign convention, which is the part students actually get wrong.

**Shortcut**

Shift+F3. F4 to lock the input references while building the amortisation row.

**How to check it**

Change the rate cell and confirm the payment moves. Then check the sign: with a positive Pv the answer is negative, which is correct and is what Excel means by money leaving. Then check that Rate and Nper agree on the period. A quick sanity test is that PMT times Nper should exceed Pv, because the excess is the interest; if it does not, the annual rate went in undivided or the years went in as years.

**Where it lands**

Week 16, sessions 31 and 32, as an addition. There is not one financial function anywhere in the syllabus, and Exercise 25 already uses PMT in its second model, the vehicle loan with data validation on the rate and the term. COBERTURA.md flags that Exercise 25 ships as .docx only, with no workbook, so the data has to be captured from scratch and week 16 does not have that time. Preparing the file in advance is the cheap fix.
...
Double-click the cell to put it in edit mode and let Excel colour-highlight the referenced ranges in place. It shows one level, only for the cell you are editing, and it disappears the moment you press Escape. Students will keep doing it because it is instant. It leaves no arrows, so an item asking to display the relationship between formulas and cells scores zero.

**Shortcut**

Ctrl+[ selects the direct precedents. Ctrl+] selects the direct dependents. Ctrl+Shift+{ selects all levels of precedents, Ctrl+Shift+} all levels of dependents. Worth knowing and worth flagging as not the graded route: they select cells, they do not draw arrows, so they answer a "select" item and fail a "trace" item.

**How to check it**

Look at the sheet. Blue arrows with dots at the source end are on screen and stay there when you click elsewhere. That persistence is the proof; nothing else in Excel leaves them behind. Count the arrowheads against the number of references in the formula bar. If the item asked for two levels, there must be two generations of arrows, not one.

**Where it lands**

Week 3, session 6, added to the session that already says "formula auditing (show formulas)" and names the group plus exactly one button. COBERTURA.md is blunt about this: the whole 3.5 block hangs off that single phrase, and Exercise 5, which covers operator precedence, references and defined names, never opens the auditing tools at all. This objective needs either an addition to Exercise 5 or class time.
...
Split the window, or freeze panes, or open a second window on the same workbook through the View tab and tile them, so the total cell stays visible while you work at the bottom of the sheet. All three are reasonable and none of them works across workbooks or survives a jump to another sheet. The Watch Window does both, and it is the only one of the four that is an objective.

**Shortcut**

None. There is no keyboard shortcut for Watch Window or for Add Watch.

**How to check it**

The pane is on screen with one row per watched cell, and the **Formula** column shows the formula text, not the value. Change a precedent and watch the **Value** column update without moving off the cell you are editing. If the watch was created but the pane was then closed, reopen it from the ribbon: the rows are still there, and that persistence is what proves a watch was added rather than a value merely being looked at.

**Where it lands**

Week 3, session 6, in the same block as 3.5.1, with the same warning: session 6 currently carries one phrase for the whole auditing group, and COBERTURA.md counts eight additions across sessions 5 and 6, so session 6 is full. The two heavier auditing objectives, 3.5.3 and 3.5.4, were pushed to week 7 for exactly this reason.
...
Sort or filter the column, spot the `#DIV/0!` by eye, and wrap the formula in IFERROR. It fixes the display and it is what gets done in real work. It also suppresses the symptom without ever visiting the rules, so a candidate who only knows IFERROR cannot answer an item that says to stop Excel flagging numbers stored as text, or to reinstate errors that were previously ignored. Teach IFERROR as the fix and the Error Checking dialog as the instrument.

**Shortcut**

None for the dialog. Ctrl+grave accent toggles **Show Formulas**, which is the fastest way to see the whole sheet as formulas while hunting for the odd one out, and it is a separate button in the same Formula Auditing group (Associate 1.4.6).

**How to check it**

After a proper sweep, the green triangles that were dismissed are gone, and so are the ones that were fixed, for a different reason. Prove which happened: go to **File**, **Options**, **Formulas** and click **Reset Ignored Errors**. Triangles that were merely dismissed come straight back; triangles that were fixed do not. That single button separates the student who understood the tool from the student who clicked Ignore Error eleven times.

**Where it lands**

Week 7, sessions 13 and 14, moved out of session 6 because session 6 was full. It sits well there: the nested IF work of session 14 is where students first generate errors worth checking, and Exercise 9 already produces `#DIV/0!` and `#VALUE!` naturally.
...
Click into the formula bar, select one fragment of the formula with the mouse, and press F9. Excel replaces the selection with its value on the spot. It is fast and genuinely useful. It is also dangerous, because pressing Enter at that moment writes the hard-coded value into the formula permanently, and it must be escaped out of with Esc, never Enter. And it answers nothing when the item asks to step into a nested reference, because F9 only evaluates what is inside the current cell. Teach F9 with the Esc warning attached, every time.

**Shortcut**

F9 on a selected fragment inside the formula bar, then Esc. Alt then M reaches the Formulas tab and the KeyTip for Evaluate Formula appears on screen.

**How to check it**

There is no artifact left on the sheet, which is exactly why the exam grades this by the dialog state. What proves the route is that the dialog was reached and driven: **Evaluation:** shows italic intermediate results rather than the untouched formula, and for a nested case the box shows the indented inner formula that only **Step In** produces. In class, the check is to have the student say out loud which part Excel underlined first and why, before clicking Evaluate. If they can predict the order they have understood precedence, and precedence is what session 6 teaches and what this tool displays.

**Where it lands**

Week 7, session 14, placed there deliberately: the nested IF of session 14 "is exactly what one debugs step by step with that tool". Exercise 8 supplies eighty-four recorded nested IF formulas and Exercise 12 supplies the IFS against nested IF comparison, so the material is already on hand.
...
Do the task by hand every time. There is no shorter route to the objective, which is the one place in this document where the graded route is also the fast one.

**Shortcut**

Alt+F8 opens the Macro dialog to run what you recorded. Alt+F11 opens the Visual Basic Editor to read it. There is no keyboard shortcut that starts the recorder; the **Shortcut key** box inside the dialog assigns a `Ctrl`+letter to the macro itself, not to recording.

**How to check it**

Press Alt+F8: the macro is in the list with **Run** available. Press Alt+F11: under the workbook's project there is a Modules folder holding a module, and inside it a `Sub` with the name you typed. The saved file name ends in `.xlsm`.

**Where it lands**

Nowhere in the 34 sessions. The word macro appears zero times in the 597 lines of the syllabus, as do VBA and Visual Basic. COBERTURA.md proposes a 90-minute workshop in the independent-work window of week 15, assembled from the TIA503 `w01` deck, whose slide "Una macro que nadie escribió a mano" is this objective with the reading of the recorded code included.
...
Accept `Macro1`, `Macro2`, `Macro3` and sort it out later. There is no later: the Macro dialog cannot rename, so sorting it out means editing the `Sub` line in the editor, which is objective 3.6.3 doing 3.6.2's work.

**Shortcut**

Alt+F8, then **Options...**, reaches the shortcut key and the description of an existing macro.

**How to check it**

Alt+F8 shows the name in the list and its description in the panel below. Alt+F11 shows the same name on the `Sub` line of the module. Press the shortcut key you assigned and the macro runs.

**Where it lands**

Nowhere in the 34 sessions. Macro workshop, with COBERTURA.md's note that `w01` of TIA503 has no naming slide but `w02` already carries the rule, "sin espacios, y no puede empezar con número", which is a line to paste onto the recording slide rather than a slide of its own.
...
Delete the macro and record it again with the mistake left out. The result is identical and the objective is not editing a macro, so nothing was demonstrated. It also loses the name, the shortcut key and the description.

**Shortcut**

Alt+F11 opens and closes the editor. Alt+F8 opens the Macro list. F7 shows the code of the selected object. Ctrl+R shows the Project Explorer. F5 runs, F8 steps one line, F9 toggles a breakpoint, Alt+Q closes the editor and returns to the sheet.

**How to check it**

Run the edited macro on a fresh copy of the data: the result matches what the recorded version produced. Once the `Select` lines are gone, the active cell does not move while the macro runs, which is the visible proof the edit took effect.

**Where it lands**

Nowhere in the 34 sessions. Macro workshop. COBERTURA.md bounds it deliberately: the TIA503 slide "La grabadora arranca el trabajo, tú lo terminas", where the line that undoes the previous line is struck out, is this whole objective on one slide, and `w06` of TIA503, "Editar lo grabado", is good material that overshoots, MO-201 asks for recording, naming and editing simple macros, not for programming.
...
Right-click the series on the chart, click **Format Data Series...**, and pick Secondary Axis. Same result, but the exam task is worded around the chart type and the axis being set together, and this route never opens the Combo entry of the dialog.

**Shortcut**

Alt+F1 embeds a chart of the default type on the current sheet; F11 puts one on its own chart sheet. Neither builds a combo. Ctrl+1 with a chart element selected opens the Format pane for that element.

**How to check it**

The plot area shows two value axes with different scales. Select the series, open **Format Selection**, and Series Options shows **Plot Series On** set to Secondary Axis. Reopen **Change Chart Type** and the Combo category is highlighted with the Secondary Axis check box still ticked for that series.

**Where it lands**

Week 11, session 21. COBERTURA.md assigns Expert 4.1.1 there and budgets it fifteen minutes of its own explanation, because it is the one item in that session that does not arrive pre-practised.
...
**Insert** tab, **Charts** group, **Recommended Charts**, and take whatever Excel offers. It will never offer Funnel or Box and Whisker, and when it does offer the right type the candidate never touched the category the task named.

**Shortcut**

Alt+F1 for an embedded chart of the default type, F11 for a chart sheet. No keystroke reaches a specific advanced type.

**How to check it**

Open **Change Chart Type**: the category on the left is highlighted with the correct subtype selected. A histogram shows bin ranges on the category axis, not the original labels. A waterfall shows its total columns anchored to zero. A sunburst shows one ring per source column.

**Where it lands**

Week 11, session 21. COBERTURA.md notes the cost here is almost zero because Exercise 17 already builds all twenty chart types in week 10; session 21 only has to criticise them and say when each one fits.
...
Tick the field check boxes and let Excel place them, or use **Insert** tab, **Tables** group, **Recommended PivotTables** and accept a suggestion. The report may come out identical and the placement decision was never made.

**Shortcut**

Press Alt, then N, then V to reach the PivotTable button through the KeyTips. Alt+D, then P, still opens the legacy PivotTable and PivotChart Wizard, which is the only route to a consolidation-range PivotTable, but it is not what the current exam expects.

**How to check it**

Clicking inside the report brings up the **PivotTable Analyze** and **Design** contextual tabs. The **PivotTable Name** box on PivotTable Analyze holds the name you typed. Each field sits in the area the task named, which you read off the four boxes at the bottom of the PivotTable Fields pane, not off the grid.

**Where it lands**

Week 14, sessions 27 and 28. COBERTURA.md already counts Expert 4.2.1 as covered by "subtotals and PivotTables"; this is the one objective of the twelve in this domain that the syllabus reaches on its own.
...
Right-click the value cell, point to **Summarize Values By**, click Average. Right-click again, point to **Show Values As**, click % of Grand Total. Then select the cells and click Percent Style on the Home tab. Three passes, three menus, and the number format is stuck to cells rather than to the field.

**Shortcut**

None. Ctrl+1 opens Format Cells for the selected cells, which is the wrong object here. Press Alt to raise the KeyTips and read the letters off the ribbon.

**How to check it**

Reopen **Field Settings**: the function is highlighted on the Summarize Values By tab and the entry is selected on the Show Values As tab. Then refresh the report. Field-level number formatting survives the refresh and reaches items that did not exist before it; cell-level formatting does not, and that difference is the whole point of the objective.

**Where it lands**

Week 14, sessions 27 and 28, listed in COBERTURA.md as "Expert 4.2.2, configuración de campo". Exercise 23 version 2 drills it four times.
...
Use the field's own filter arrow in the Rows area, or drag the field into the Filters area and use its drop-down. It filters the same report. It leaves no object on the sheet, so a task that says "insert a slicer" scores nothing.

**Shortcut**

With a slicer selected, Alt+C clears its filter. Ctrl+click adds items to the selection without turning Multi Select on.

**How to check it**

The slicer is a floating object above the grid with the field name in its header. Selected buttons are shaded, and the funnel-with-a-cross icon in the header becomes available once a filter is on. Verified through the object model: creating one adds a slicer cache named `Slicer_<FieldName>` and the slicer defaults to one column of buttons.

**Where it lands**

Week 15, sessions 29 and 30. COBERTURA.md moved Expert 4.2.3 out of week 14 into the advanced-filters week on the reasoning that a slicer is a filter with buttons, and because week 14 could not hold it.
...
Right-click a date item and click **Group**. It opens the same dialog, but the exam wording points at the **Group** group of the PivotTable Analyze tab. Worse, in Microsoft 365 dropping a date field into Rows auto-groups it into Years, Quarters and Months without anyone touching a dialog, so a candidate can see the right answer on screen having done nothing, and score nothing.

**Shortcut**

Alt+Shift+Right Arrow groups, Alt+Shift+Left Arrow ungroups. These are the worksheet outline keys and they are reported to reach the PivotTable Group commands too, **TO CONFIRM** inside a PivotTable on this build.

**How to check it**

The PivotTable Fields pane holds one field per grouped level that was not there before, which is the fingerprint the graded route leaves. Reopen **Group Field** and the levels are still highlighted in the **By** list. Verified through the object model: grouping a date field by Months, Quarters and Years adds Quarters and Years to the field collection.

**Where it lands**

Week 14, sessions 27 and 28, listed in COBERTURA.md as "Expert 4.2.4, agrupar datos de la tabla dinámica por fechas, por rangos numéricos y por selección".
...
Add a helper column to the source data with `=Units*Price`, refresh the PivotTable, and drag the new column into Values. The arithmetic is actually more correct this way, and it earns nothing when the task says calculated field, because no calculated field exists in the workbook.
...
The field is listed in the PivotTable Fields pane but is not a column in the source data. Reopen **Insert Calculated Field** and the **Name** drop-down holds it. Verified through the object model: adding `= Units * Price` under the name Revenue produces a field called Revenue that arrives in Values captioned "Sum of Revenue".

One warning to hand the students, because it is the most common wrong answer in this objective. A calculated field is evaluated against the **totals** of each field inside each cell of the report, not row by row. `Units * Price` therefore returns Sum(Units) multiplied by Sum(Price) for each group, which is not the sum of the line amounts. Cross-check one cell against a helper column: if the two disagree, the field is multiplying totals and the model needs the helper column in the source instead.

**Where it lands**

Week 14, sessions 27 and 28. COBERTURA.md flags this one specifically: if the professor keeps slicers and PivotCharts in week 14 instead of moving them to week 15, 4.2.5 is the objective that falls out, and the document recommends against that.
...
Select the value cells and click Percent Style, Comma Style or Increase Decimal on the **Home** tab, **Number** group. Or select the whole report and apply a cell style. Both look right on screen and both are attached to cells.

**Shortcut**

Ctrl+1 opens Format Cells for the selected cells. In this objective that is precisely the ungraded route, because it formats cells rather than the field. Alt+F5 refreshes the PivotTable, Ctrl+Alt+F5 refreshes everything in the workbook.

**How to check it**

Refresh the report with Alt+F5. Formatting applied through Field Settings survives and reaches items that appeared for the first time in the refresh; formatting applied cell by cell drifts or disappears. Verified through the object model: the style, the banding, the report layout, the subtotal setting, the grand totals and the preserve-formatting flag are all separate properties of the PivotTable, which is why the exam can grade them separately.

**Where it lands**

Week 14, sessions 27 and 28, listed in COBERTURA.md as "Expert 4.2.6, formato de la tabla dinámica", with the note that MO-211 rewrote this objective as "configure value field settings", which is the same dialog as 4.2.2.
...
Select the PivotTable's cells and use **Insert** tab, **Charts** group, like an ordinary range. Excel usually still produces a PivotChart, but the moment the candidate copies the report to a static range first, what comes out is a plain chart with no field buttons and no link to the report.

**Shortcut**

Alt+F1 embeds a chart from the active PivotTable on the current sheet; F11 puts one on a new chart sheet (**TO CONFIRM** that F11 produces a PivotChart rather than a static chart on this build).

**How to check it**

Field buttons sit on the chart. Clicking it raises the **PivotChart Analyze** contextual tab, which a plain chart does not have. Change the PivotTable layout and the chart follows immediately. Verified through the object model: `Chart.HasPivotFields` returns True and a `PivotLayout` object exists.

**Where it lands**

Week 15, sessions 29 and 30. COBERTURA.md notes that charts and PivotTables are taught apart in weeks 10, 11 and 14 and are never joined, which is why the whole 4.3 block was pushed here.
...
Right-click the chart and use Change Chart Type from the context menu. Right-click a field button and click Hide, which removes that one button but not the category the task named. Or use the three floating buttons beside the selected chart, the plus, the paintbrush and the funnel.

**Shortcut**

Ctrl+1 opens the Format pane for whichever chart element is selected. Alt+F5 refreshes. Arrow keys move the selection between chart elements once one is selected, which is the reliable way to reach a series you cannot click.

**How to check it**

The field buttons that were switched off are gone while the rest remain, and the chart still filters from the PivotTable. The PivotChart Fields pane still shows the four PivotChart area names. After Switch Row/Column, the PivotTable underneath has swapped its Rows and Columns, not just the chart. Verified through the object model: `ShowAllFieldButtons` is a chart-level property, and changing `ChartType` from clustered column to line left `HasPivotFields` True.

**Where it lands**

Week 15, sessions 29 and 30, listed in COBERTURA.md as "Expert 4.3.2, opciones del gráfico dinámico: botones de campo, cambiar tipo, filtros del gráfico".
...
Click the paintbrush icon floating beside the selected chart and pick from the Style and Color tabs of the flyout. Same styles, same colours, two clicks, and the ribbon path the task described was never opened.

**Shortcut**

Ctrl+1 with an element selected opens its Format pane. There is no keystroke for the style gallery.

**How to check it**

Reopen the **Chart Styles** gallery and the applied style is boxed. Reopen **Format Selection** and the pane comes up on the element you styled with your settings intact. Verified through the object model: `Chart.ChartStyle` on a PivotChart accepts a style number and reads back the same number.

**Where it lands**

Week 15, sessions 29 and 30, listed in COBERTURA.md as "Expert 4.3.3, estilos del gráfico dinámico".
...
Right-click the value cell, point to **Show Details**, and let Excel produce the sheet. Or filter the source data by hand to see the same rows. The second one produces the same information and leaves no drill-through sheet, so a task worded "show the details behind" scores nothing.

**Shortcut**

Double-clicking the value cell is the drill; there is no keystroke for it. Alt+Shift+Right Arrow and Alt+Shift+Left Arrow expand and collapse the active field in the report (**TO CONFIRM** on this build, the same pair as 4.2.4).

**How to check it**

A brand new worksheet appears in the workbook holding just the rows behind that one cell. Count its rows, or sum its amount column, against the number you double-clicked; they have to agree. Verified through the object model: setting `ShowDetail` to True on a value cell took the workbook from two sheets to three and made the new sheet active.

**Where it lands**

Week 15, sessions 29 and 30, listed in COBERTURA.md as "Expert 4.3.4, profundizar en el detalle, expandir y contraer niveles". Exercise 23 version 2, with its 26,608 lines of restaurant tickets, is the only file in the package big enough for the drill-through sheet to mean anything.
